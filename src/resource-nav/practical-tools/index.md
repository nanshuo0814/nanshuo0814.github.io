---
article: false
---

# 实用资源工具导航

::: tip 说明
收集常见常用的实用工具资源

倘若你知晓其他更好的资源，而这些资源在本站尚未收录，欢迎随时 <a href="/about/">**联系站长**</a> 分享贡献资源哦！

如果你有任何建议或问题，请随时<a href="/about-author/">**联系站长**</a>
:::

## 免费影视网站&APP
```component VPCard
{
        "title": "搜片网站线路网速情况",
        "logo": "https://soupian.one/images/icon.png",
        "desc": "查看搜片网站不同域名下线路的网速情况，找到最优的进行观看",
        "link": "https://xindizhi.github.io/soupian/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```

```component VPCard
{
        "logo": "https://soupian.one/images/icon.png",
        "title": "搜片",
        "desc": "收录每个优秀的影视站点，集合各种影视资源，方便你快速找到你喜欢的片源",
        "link": "https://soupian.one/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://dyxs31.com/statics/logo.png",
        "title": "电影先生",
        "desc": "电影、电视剧、综艺、动漫等等都能看，不错",
        "link": "https://dyxs31.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://waipian28.com//statics/logo.png",
        "title": "歪片星球",
        "desc": "电影、电视剧、综艺、动漫等等都能看，还行",
        "link": "https://waipian28.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.pptv06.com/mxtheme/images/favicon.png",
        "title": "皮皮影视",
        "desc": "电影、电视剧、综艺、动漫等等都能看，相对不错",
        "link": "https://www.pptv06.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://tse4-mm.cn.bing.net/th/id/OIP-C.lT6aPKhFwKX1UHwT5fdT_QAAAA?rs=1&pid=ImgDetMain",
        "title": "往往影视",
        "desc": "精彩电影剧集免费在线观看",
        "link": "https://www.oftens.top/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://registry.npmmirror.com/@liumingye/cdn2/1.5.63/files/favicon.ico",
        "title": "MyFreeMP3",
        "desc": "免费音乐在线听和下载mp3音频和歌词",
        "link": "https://tools.liumingye.cn/music/#/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "http://www.eev3.com/images/logo.png",
        "title": "易听音乐网",
        "desc": "免费音乐在线听和下载mp3音频和歌词",
        "link": "http://www.eev3.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```


## 知名搜索引擎
```component VPCard
{
        "logo": "https://www.baidu.com/favicon.ico",
        "title": "百度",
        "desc": "百度一下，你就知道",
        "link": "https://www.baidu.com",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.google.com/favicon.ico",
        "title": "Google",
        "desc": "全球最大的搜索引擎",
        "link": "https://www.google.com",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.bing.com/favicon.ico",
        "title": "Bing",
        "desc": "微软 Bing 搜索",
        "link": "https://cn.bing.com",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.sogou.com/favicon.ico",
        "title": "搜狗",
        "desc": "搜狗搜索",
        "link": "https://www.sogou.com",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.so.com/favicon.ico",
        "title": "360",
        "desc": "360 搜索",
        "link": "https://www.so.com",
    "background": "rgba(253, 230, 138, 0.15)"  }
```

## 在线翻译网站
```component VPCard
{
        "logo": "https://www.baidu.com/favicon.ico",
        "title": "百度翻译",
        "desc": "百度翻译，翻译工具",
        "link": "https://fanyi.baidu.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.so.com/favicon.ico",
        "title": "360翻译",
        "desc": "360 翻译，翻译工具",
        "link": "https://fanyi.so.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.google.com/favicon.ico",
        "title": "Google翻译",
        "desc": "谷歌翻译，翻译工具",
        "link": "https://translate.google.cn/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.sogou.com/favicon.ico",
        "title": "搜狗翻译",
        "desc": "搜狗翻译，翻译工具",
        "link": "https://fanyi.sogou.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.bing.com/favicon.ico",
        "title": "Bing翻译",
        "desc": "微软 Bing 翻译，翻译工具",
        "link": "https://cn.bing.com/translator",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.deepl.com/favicon.ico",
        "title": "Deepl翻译",
        "desc": "Deepl翻译，翻译工具",
        "link": "https://www.deepl.com/translator",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.iciba.com/favicon.ico",
        "title": "ICIBA翻译",
        "desc": "ICIBA翻译，翻译工具",
        "link": "https://www.iciba.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.youdao.com/favicon.ico",
        "title": "有道翻译",
        "desc": "有道翻译，翻译工具",
        "link": "https://fanyi.youdao.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```

## 远程办公软件&网站
```component VPCard
{
        "title": "向日葵远程控制",
        "logo": "https://sunlogin.oray.com/favicon.ico",
        "desc": "支持多系统设备，实现跨平台互控，提供安全、高效的远程管理服务",
        "link": "https://sunlogin.oray.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.todesk.com/favicon.ico",
        "title": "ToDesk",
        "desc": "远程控制软件，提供高效稳定、安全加密的多屏高清远程桌面服务",
        "link": "https://www.todesk.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "data:image/x-icon;base64,",
        "title": "苏打办公",
        "desc": "一站式企业协作平台，提供文档编辑、会议、邮件等多功能服务",
        "link": "https://bangong.360.cn/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://wormhole.app/images/logo-80x80.png",
        "title": "Wormhole",
        "desc": "Wormhole 让你能以端到端加密和自动过期链接的方式分享文件",
        "link": "https://wormhole.app/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://static.deepl.com/img/favicon/favicon_16.png",
        "title": "Deepl文档翻译器",
        "desc": "包括PDF、word、pptx格式的在线翻译",
        "link": "https://www.deepl.com/translator/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
## 邮箱
```component VPCard
{
        "logo": "https://mail.163.com/favicon.ico",
        "title": "网易邮箱",
        "desc": "163邮箱是网易提供的稳定、大容量的电子邮件服务",
        "link": "https://mail.163.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://mail.proton.me//assets/apple-touch-icon-60x60.png",
        "title": "ProtonMail邮箱",
        "desc": "一个注重隐私保护的电子邮件服务，提供端到端加密、无日志政策和基于瑞士法律的安全性",
        "link": "https://mail.google.com/mail/u/1/?pli=1#inbox",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico",
        "title": "Gmail邮箱",
        "desc": "Google提供的免费电子邮件服务，以强大的搜索、大容量存储、标签系统",
        "link": "https://mail.google.com/mail/u/1/?pli=1#inbox",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "title": "QQ邮箱",
        "logo": "https://mail.qq.com/zh_CN/htmledition/images/favicon/qqmail_favicon_32h.png",
        "desc": "腾讯提供的免费邮箱服务，以简洁界面、大容量存储、与QQ集成和安全性著称",
        "link": "https://mail.qq.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
## 图片相关
```component VPCard
{
        "title": "removebg",
        "logo": "https://www.remove.bg//apple-touch-icon.png?v=fc0bfce6e1310f1539afec9729716721",
        "desc": "一个使用AI自动去除图片背景的在线服务，快速、准确且易于使用",
        "link": "https://www.remove.bg/zh/upload",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.vidmore.com//images/index/favicon.ico",
        "title": "vidmore",
        "desc": "在线水印去除器可以完美地消除图像中的水印",
        "link": "https://www.vidmore.com/zh/watermark-remover/#",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://arc.tencent.com//logo.ico",
        "title": "ARC",
        "desc": "腾讯旗下的AI图片处理工具，修复模糊图片",
        "link": "https://arc.tencent.com/zh/ai-demos/faceRestoration",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
## 资源搜索
```component VPCard
{
        "title": "阿里网盘资源共享站",
        "logo": "https://img.alicdn.com/imgextra/i2/O1CN01DOYcs71v3B6bOemVM_!!6000000006116-2-tps-512-512.png",
        "desc": "人人为我，我为人人的共享资源社区",
        "link": "https://pan666.net/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.chongbuluo.com/static/image/common/logo-light-icon.svg",
        "title": "虫部落",
        "desc": "专注于资源分享的平台，无论是找学习资源还是娱乐资源",
        "link": "https://www.chongbuluo.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
```component VPCard
{
        "logo": "https://www.xiaobaipan.com/static/assets/ico/favicon.png",
        "title": "小白盘",
        "desc": "让你的云端储存更便捷，提供高效的 云存储解决方案",
        "link": "https://www.xiaobaipan.com/",
    "background": "rgba(253, 230, 138, 0.15)"  }
```
<br />

::: warning 版权声明
本文由 [南烁](https://www.nanshuo.icu) 开发，如有引用、借鉴的请保留版权声明
:::
